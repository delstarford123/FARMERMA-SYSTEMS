...
##########################
# FARMERS ACADEMY (COMING SOON)
##########################

@app.route('/academy')
@login_required # If you have a login system
def academy_home():
    # You can fetch course data from Firebase here later
    return render_template('academy/index.html')

@app.route('/academy/course/<course_id>/<lesson_id>')
def view_lesson(course_id, lesson_id):
    # Example: Fetch lesson details from Firebase
    lesson_data = rtdb.reference(f'courses/{course_id}/lessons/{lesson_id}').get()
    
    # Example Meet link (In production, this could be specific to the course)
    meet_link = "https://meet.google.com/abc-defg-hij" 
    
    return render_template('academy/course_view.html', 
                           lesson=lesson_data, 
                           meet_link=meet_link)
